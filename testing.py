...
cases = [
    {
        ...
        'stable_points': [ (1200, 0)],
        'unstable_points': [(0, 0), (0, 400)], 
        'special_initial_conditions': [(200, 200), (50, 400), (800, 400)]
    },

    {
        ...
        'stable_points': [(0, 1200)],
        'unstable_points': [(500, 0), (0, 0)], 
        'special_initial_conditions': [(100, 400), (500, 100), (400, 1000)]
    },

    {
        'r1': 0.1, 'K1': 1200, 'alpha12': 2.7, 'r2': 0.1, 'K2': 800, 'alpha21': 1.5,
        'N1_values': np.linspace(0, 1400, 10), 'N2_values': np.linspace(0, 800, 10),
        'ylim': (0, 800), 'xlim': (0, 1400), 'equilibrium_points': [(0, 0), (1200, 0), (0, 800), (19200/61, 20000/61)],
        'stable_points': [(1200, 0)],
        'unstable_points': [(0, 0), (19200/61, 20000/61), (0, 800)],
        'special_initial_conditions': [(800, 700), (100, 400), (400, 250), (200,200)]
    },

    {
        'r1': 0.1, 'K1': 1500, 'alpha12': 1.5, 'r2': 0.1, 'K2': 800, 'alpha21': 0.4,
        'N1_values': np.linspace(0, 2100, 10), 'N2_values': np.linspace(0, 1200, 10),
        'ylim': (0, 1200), 'xlim': (0, 2100), 'equilibrium_points': [(0, 0), (1500, 0), (0, 800), (750, 500)],
        'stable_points': [(750, 500)],
        'unstable_points': [(0, 0), (1500, 0), (0, 800)], 
        'special_initial_conditions': [(250, 400), (100, 800), (1500, 100), (1000, 800)]
    }
]
...
for i, case in enumerate(cases):
    ...
    plot_vector_field(K1, K2, r1, r2, alpha12, alpha21, case['N1_values'], case['N2_values'], axs[row, col])

    # for each special initial condition, plot the trajectory with red and with the arrows
    for initial_condition in case['special_initial_conditions']:
        solutions = runge_kutta_ode_sys(h, odes, initial_condition, t0, t_max)
        axs[row, col].plot(solutions[:, 0], solutions[:, 1], color='red')
        for i in range(0, len(solutions) - 1, max(1, len(solutions) // 2)):
            axs[row, col].arrow(solutions[i, 0], solutions[i, 1], 
                      solutions[i+1, 0] - solutions[i, 0], 
                      solutions[i+1, 1] - solutions[i, 1], 
                      head_width= 15, head_length=20, color='red')

    axs[row, col].legend()
    axs[row, col].set_ylim(case['ylim'])
    axs[row, col].set_xlim(case['xlim'])
    axs[row, col].set_title(f'Caso {i+1}')
